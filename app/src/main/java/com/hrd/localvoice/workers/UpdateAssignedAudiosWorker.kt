...
class UpdateAssignedAudiosWorker(
    ...
    override fun doWork(): Result {
        ...
        apiService?.getAssignedAudios(!completed)?.enqueue(object : Callback<ValidationAudiosResponse?> {
            override fun onResponse(
                call: Call<ValidationAudiosResponse?>, response: Response<ValidationAudiosResponse?>
            ) {
                    ...
                    if (audios?.isNotEmpty() == true) {
                        audios.forEach { audio ->
                            ...
                            if (alreadyExistingAudio.localAudioUrl?.let { File(it).exists() } != true) {
                                ...
                                val title =
                                    audio.remoteAudioUrl.split("/")[audio.remoteAudioUrl.split("/").size - 1]
                            ...
                            if (alreadyExistingAudio.localImageUrl?.let { File(it).exists() } != true) {
                                // Local file does not exist
                                val title =
                                    audio.remoteImageUrl.split("/")[audio.remoteImageUrl.split("/").size - 1]
